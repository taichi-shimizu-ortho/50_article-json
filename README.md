...
**読み上げ (Web Speech API、依存なし)。** ヘッダの `▶ Read` で現在のページの
先頭から最後のセクションまで連続で読む — ページは自動でめくり、読んでいる
段落をハイライトして追従する。段落の `🔊` はその段落だけ読む (もう一度押すと
停止)。速度はヘッダのセレクタ (0.8×〜2×)。本文は英語なので `en-US` の音声を
明示的に選ぶ (既定音声が日本語の環境で化けないように)。**文単位で utterance を
分ける** — Chrome は長い utterance を数十秒で黙って打ち切ることがあり、
文分割は JSON の `sentences` からもらえる。手動でページを移ると読み上げは
止まる (読み上げ側のページ送りでは止めない)。図表キャプションと References は
読まない。
