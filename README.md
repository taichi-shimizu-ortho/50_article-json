...
**普段は `npm run serve` だけでよい。** ビューアが初回に同意ダイアログを出し
(CLI の `--accept-license` に相当。同意は `data/private/consent.json` に端末ごとに記録)、
以降は起動のたびに不足分を自動で取得して build まで済ませる。以下の CLI は
同じことをターミナルからやりたいとき用。Windows の PowerShell と macOS の
ターミナルで同じコマンドを使える。
...
抽出元が表を **Markdown のパイプ表** (`| a | b |` + `| --- |`) の複数行文字列
として段落に入れてくることがある。これは `<p>` に潰す**前に** `<table-wrap>` に
変える — 潰すと改行が消えて、`| |` が「行の継ぎ目」か「空セル」かを二度と
区別できなくなる。直前の段落が `Table N .` ならキャプションとして取り込み、
本文の `Table N` への言及は `<xref ref-type="table">` にする (ビューアが表を
参照段落の直後に置ける)。
...
src/viewer/index.html         ビューアのマークアップと CSS
src/viewer/app.js             ビューアのクライアント (素の ES Module)
...
**論文はセクションごとに 1 ページ。** 左の目次 (Abstract / Introduction / … /
References) か、ページ下部の前へ/次へ、または ← → キーで移動する。入れ子の
セクションは親と同じページに出す。謝辞・利益相反などの短い後付けは
`Back matter` の 1 ページにまとめ、文献表は `References` ページとして一覧できる
(本文のホバー書誌と同じデータ)。URL は `#a/{id}/{page}` なので、ページ位置ごと
リロード・共有できる。ページ位置も localStorage に残る。

**Library 画面** (ヘッダの `Library`) が目録 (corpus) と取得状況の一覧。
論文ごとに 取得済み / XML のみ / 未取得 (自動取得可) / 未取得 (手動) の
バッジが付き、不足分の一括取得・全論文の図版取得もここから実行できる。
自動取得できない論文 (PMCID が無い import-json 由来など) は取得元 URL を示す。

**不足分は起動時に自動で取得する。** 初回だけ同意ダイアログを挟み
(上の「再配布不可の論文を各端末で取得する」を参照)、同意が記録済みなら
以降は黙って取得・build してヘッダに進捗を出す。

段落サマリー (`gist` / `plain`) があれば本文の上に出し、`Full text` / `Plain`
ボタンで隠せる。文パスを回してあれば certainty も重なる。**UI は英語。**

**読み上げ (Web Speech API、依存なし)。** ヘッダの `Read aloud` で現在のページの
先頭から最後のセクションまで連続で読む — ページは自動でめくり、読んでいる
段落をハイライトして追従する。`Pause` / `Resume` / `Stop` で操作し、速度は
セレクタ (0.8×〜2×、既定 0.95×)。段落の `🔊` はその段落だけ読む (もう一度
押すと停止)。状態はヘッダの `aria-live` 領域に出るので、どの音声で読んでいるか
が画面にもスクリーンリーダーにも伝わる。

**音声は Google US English を名指しで優先する** (Chrome が音声一覧で公開して
いて、手元の OS 音声より医学英語の読みが安定している)。入っていない端末では
en-US → en と落とす。テキストがブラウザの外に出ることはない。

**文単位で utterance を分ける** — Chrome は長い utterance を数十秒で黙って
打ち切ることがあり、文分割は JSON の `sentences` からもらえる。それでも
240 字を超える文は語境界で割る。手動でページを移ると読み上げは止まる
(読み上げ側のページ送りでは止めない)。図表キャプションと References は読まない。

実装で気をつけている点が 2 つある。**`synth.paused` を直接見ない** — `pause()`
の直後に読んでも false のままの実装があり、ボタンが `Pause` のまま固まって
再開の手段が画面から消える。**`cancel()` の前に `resume()` する** — 一時停止中に
cancel すると停止状態が残り、次の `speak()` が鳴らないまま無反応になる。

図版と表は**それを最初に参照した段落の直後**に入る (`Figures` / `Tables`
ボタンで消せる)。`article.figures` / `article.tables` は本文ツリーの外にあるので、
論文の読み込み時に本文全体を歩いて「どの段落の直後に出すか」を先に決めておく
(ページを分けても行き先が変わらないようにするため)。どの段落からも参照されない
ものは `Figures & Tables` ページにまとめる (落とさない)。
...
**`Enrich…` ボタンで LLM 付与も UI から実行できる。** 課金する処理なので、
実行前に `GET /api/enrich/estimate` の見積もり (対象段落数・キャッシュ済み数・
API リクエスト数) をダイアログに出し、確認してから `POST /api/jobs/enrich` を
叩く。API キーはサーバ側で `.env` から読む — 無ければ見積もりの時点で
その旨を出し、実行ボタンを無効にする。

**書き込みの口 (`/api/jobs/*`, `/api/figures/{id}`, `/api/consent`) だけが
外に出て行き、ファイルを書く。** 読むだけの他の口とは性質が違うので条件を
絞ってある — POST のみ / 他オリジンからは 403 / id は目録・実在ファイルから
引けるものだけ / ジョブは全体で同時に 1 本だけ。127.0.0.1 に bind
